...
void dualres::GPMCommandParser<T>::show_details() const {
  std::cerr << "\nOther Details \n"
	    ...
	    << "    the {marginal variance; bandwidth; exponent}. The marginal \n"
	    << "    variance parameter is also sometimes referred to as the 'partial' \n"
	    << "    sill. dualgpm[f] will estimate these from the data if not \n"
	    << "    provided, but for finer user control, please see our other program \n"
	    << "    program estimate_rbf. \n"
	    << "\n"
	    << "--neighborhood \n"
