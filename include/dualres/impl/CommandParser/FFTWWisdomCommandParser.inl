...
void dualres::FFTWWisdomCommandParser<T>::show_help() const {
  ...
  std::cerr << "Options:\n"
	    ...
	    << "be saved to a temporary file so that subsequent FFTs are computed \n"
	    << "very efficiently. These files are small and typically removed by the \n"
	    << "operating system if they're not accessed within X number of days. \n"
	    << "\n"
